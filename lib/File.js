...
File.prototype.index = function ( opt, cb ) {
	...
	function recurseDir ( cb ) {
		if ( index.meta && index.meta.type == 'dir' && opt.recurse > 0 ) {
			...
			var walkOpt = _.clone( opt );
			walkOpt.depth = 1;
			walkOpt.includeSelf = false;

			file.walk( walkOpt, function onFileReaddir ( err, listing ) {
				if ( err ) {
					cb( err );
					return;
				}
...
`file.meta ( cb( err, meta ) )`
`file.meta ( options, cb( err, meta ) {} )`

### Options

See (Meta)[Meta.md] for options. 
...
Returns a `File` object representing a file with a path relative to this file's path.
...
Returns a canonical url where this file can be retrieved, or `undefined` if this operation is not possible.
*/
...
Returns a `FileIterator` object.

### Options
- `dirs`: Whether to include directories in output. ( Default: `true` )
- `files`: Whether to include files in output. ( Default: `true` )
- `depth`: The depth to which to recurse subdirectories. ( Default: unlimited )
- `includeSelf`: Whether this file is included in output. ( Default: `false` ) 

*/

File.prototype.walk = function ( opt, cb ) {
	if ( 'function' == typeof opt ) {
		cb = opt;
		opt = {};
	}
	...
	var iterator;
	if ( opt instanceof FileIterator ) {
		iterator = opt;
		opt = iterator.recurseOpt || iterator.opt;
	} else {
		opt = opt || {};

		iterator = new FileIterator ( opt, cb );
		cb = finish;
	}
	...
	file.stat( function ( err, stat ) {
		...
		if ( stat.isDir ) {
			...
			if ( opt.depth <= 0 ) {
				...
				if ( !iterator.recurseOpt ) {
					iterator.recurseOpt = _.clone( opt );
					iterator.recurseOpt.includeSelf = false;
					opt = iterator.recurseOpt;
				}
...
File.prototype.mkdir = function ( cb )
{
