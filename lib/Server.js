...
Server.prototype.middleware = function ()
{
	...
	return function ( req, res ) {
		...
		function parseQuery ( cb ) {
			if ( !path || path == '/' && req.query.path ) {

				path = req.query.path;
				console.warn( "PATH OVERRIDE", path );
		...
		function findFile ( cb ) {
			...
			console.warn ( "FIND FILE", path );

			if ( !file ) {
				// If the file is missing here, it means that there is no matching
				// stream.
		...
		function doAction ( cb ) {
			...
			switch ( action ) {
				case 'readdir':
					file.readdir( function onFileReaddir ( err, listing ) {
						if ( err ) {
							cb( err );
						} else {
							res.send( listing );
				...
				case 'index':
					...
					file.index( opt, function onFileIndex ( err, index ) {
						res.send( index );
						cb( true );
					});
				break;
				...
				default:
					cb( Server.Errors.InvalidAction( action ) );
				break;
			}

			/*
			var sendIndex = !!req.query.index || req.query.index === "";

			//console.warn ( "Server.sendIndex", req.query );
			if ( sendIndex ) {
		...
		function sendDirListing ( cb ) {
			if ( file && file.info.type == 'dir' ) {
				file.index( 
					{ recurse: 1 }, 
		...
		function sendRelatives ( cb ) {
			if ( file ) {
				file.relatives( 
					{ exists: true }, 
					function onFileRelatives ( err, relatives ) {
						for ( var streamName in relatives ) {
							var relFile = relatives[streamName];
							res.set( 'Wildcat-'+streamName, relFile.url() );
						}				
					}
				);
			}
...
*/
				...
				readString( function ( err, str ) {
					...
					linkSrc.stat ( function ( err, info ) {
						if ( err ) {
							...
							storeOpt.link = true;
							file.store( linkSrc, storeOpt, fileStored );
						}
					});
				} );
			}
		...
		function sendStatus ( cb ) {
			switch ( statusCode ) {
				...
				default:
					console.warn( "Invalid Code", statusCode );
