	...
	function initStreams ( cb ) {
		if ( config.streams ) {
			for ( var name in config.streams ) {
				var stream = router.addStream( name, config.streams[name] );
				if ( String( stream.path ) == '**' && !router.defaultStream )
					router.defaultStream = stream;
			}
		}

		if ( !router.defaultStream ) {
			router.defaultStream = router.addStream( "**", {} );
		}
