...
Router.open = function ( filename, cb ) {
	Config.loadFromUrl ( filename, function ( err, conf ) {
		if ( err )
			return cb( err );

		var router = Router( conf.config );
		router.init( cb );
	});
}
...
Router.prototype.init = function ( cb )
{
	var router = this,
		...
		console.log ( "Router init", config );
...
Router.prototype.configPublic = function () {
	var router = this,
...
Router.prototype.configLocal = function () {
	var router = this,
		streams = router.streams,
		conf = {};
	...
	_.map( streams, function ( stream, name ) {
		conf.streams[name] = stream.configLocal();
	});

	if ( router.server ) {
		conf.server = router.server.configLocal();
	}

	if ( router.storage ) {
		conf.storage = router.storage.configLocal();
	}
