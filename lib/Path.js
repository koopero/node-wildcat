...
function Path ( parse ) {
	...
	if ( 'object' == typeof parse && parse.path )
		parse = String( parse.path );
...
Path.prototype.startsWith = function ( b ) {
	var path = this,
		str = path.str;
	...
	if ( str.substr( 0, b.length ) == b ) {
		return Path( str.substr( b.length ) );
	}
}

Path.prototype.endsWith = function ( tail )
{
	var str = this.str;
...
Path.prototype.dirname = function () {
	var path = this,
		str = this.str,
		lastSlash = str.lastIndexOf( '/', str.length - 2 );
