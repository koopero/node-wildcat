var 
	...
	LocalFile = require('./Storage/LocalFile.js'),
	Utils = require('./Utils.js');

module.exports = Meta;

function Meta( file, options, cb ) {
	...
	function ffprobe ( meta, cb ) {
		...
		commandJson( [
			...
			if ( result.streams ) {
				result.streams.forEach ( function ( stream ) {
					if ( stream['codec_type'] == 'video' && !hasVideo ) {
						hasVideo = true;
						ret['video-codec'] = stream["codec_name"];
						ret['image-width'] = stream['width'];
						ret['image-height'] = stream['height'];
					}

					if ( stream['codec_type'] == 'audio' && !hasAudio) {
						hasAudio = true;
						ret['audio-codec'] = stream['codec_name'];
						if ( stream['sample_rate'] )
							ret['audio-samplerate'] = parseInt( stream['sample_rate'] );
					}
				});
			}
			...
			if ( result.format ) {
				...
				ret['media-duration'] = parseFloat( format['duration'] );
			}
			...
			cb( null, { meta: ret, details: { "ffprobe": true } } );
		});
	}
	...
	function command( builder, cb ) {
		var inputFile = new LocalFile ( '' );
		inputFile.localPath = file; 
