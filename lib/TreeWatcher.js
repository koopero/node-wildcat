...
TreeWatcher.prototype.addDirectory = function ( path, emit )
{
	...
	fs.readdir( dirPath, function ( err, listing ) {
		...
		async.each( listing, function ( filename, callback ) {
			...
			fs.lstat( that.fullPath( filePath ), function ( err, stat ) {
				if ( stat.isDirectory() ) {
					if ( emit ) {
				...
				} else if ( ( stat.isSymbolicLink() || stat.isFile ()) && that.rememberFiles ) {
					if ( node.files )
						node.files.push( filename );
					else 
						node.files = [ filename ];
		...
		var watcher = fs.watch( dirPath, { persistent: that.persistent } );
